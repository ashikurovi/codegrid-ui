                    ...
                    <div className="flex items-center gap-4">
                      {item.product?.images?.[0] ? (
                        <div className="w-12 h-12 bg-gray-100 rounded overflow-hidden flex-shrink-0">
                          <img src={item.product.images[0].startsWith('http') ? item.product.images[0] : `http://localhost:8000${item.product.images[0].startsWith('/')?'':'/'}${item.product.images[0]}`} alt={item.product.name} className="w-full h-full object-cover" />
                        </div>
                      ) : (
