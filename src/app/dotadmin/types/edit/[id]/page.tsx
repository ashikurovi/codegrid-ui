...
import { getTypeById, updateType } from "../../../../../api/typeApi";
...
export default function EditTypePage() {
  ...
  const fetchTypeData = async () => {
    try {
      const res = await getTypeById(typeId as string);
      if (res.data) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateType(typeId as string, {
        name,
        description: "" // keep backend happy
      });
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-gray-500">Loading type...</div>;
  }
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-3xl font-black uppercase tracking-tight text-black">Edit Type: {name}</h1>
      </div>
        ...
        <div className="flex-1 w-full border-[3px] border-black bg-white p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] rounded-none">
          <form id="typeForm" onSubmit={handleSubmit} className="flex flex-col gap-8">
            ...
            <div className="mt-4 flex gap-4">
              ...
              >
                {isSubmitting ? "Saving..." : "Update Type"}
              </button>
              <button 
                type="button"
